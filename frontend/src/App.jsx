...
function initParticles(canvas) {
  ...
  function draw() {
    ...
    for (const p of particles) {
      ...
      ctx.fillStyle = dark ? `rgba(74,222,128,${p.a})` : `rgba(34,197,94,${p.a * 0.6})`;
      ctx.fill();
    }
...
export default function App() {
  ...
  useEffect(() => {
    api.health().then(() => setApiAvailable(true)).catch(() => setApiAvailable(false));
  }, []);

  const handleResult = useCallback((record) => {
    addRecord({
  ...
  return (
    <>
      ...
      <Navbar dark={dark} onToggleTheme={() => setDark((d) => !d)} apiAvailable={apiAvailable} />
      ...
      <section className="hero" aria-labelledby="hero-heading">
        <div className="hero-content">
          ...
          <h1 id="hero-heading" className="hero-title">
            Track Your <span className="gradient-text">Carbon Impact</span> Daily
          </h1>
          ...
          <button className="hero-cta" onClick={() => document.getElementById("activityForm")?.scrollIntoView({ behavior: "smooth" })}>
            🌿 Start Tracking
          </button>
        </div>
        ...
        <section className="panel panel-form" aria-label="Log Activity">
          <div className="panel-header">
            ...
            <div>
              ...
              <div className="panel-sub">Enter your daily transport, energy & diet</div>
            </div>
          </div>
          <FootprintForm onResult={handleResult} onTips={handleTips} apiAvailable={apiAvailable} />

          {tips && tips.tips?.length > 0 && (
            <div className="tips-container" id="tips">
              <div className="tips-heading">💡 Personalised Tips</div>
              <ul className="tips-list">
                {tips.tips.map((t, i) => <li key={i} className="tips-item">{t}</li>)}
              </ul>
            </div>
          )}
          ...
          {history.length > 0 && (
            <div style={{ marginTop: "1rem", textAlign: "right" }}>
              <button className="btn-ghost" onClick={clearAll}>🗑️ Clear History</button>
            </div>
          )}
        </section>

        {/* Chart panel */}
        <section className="panel panel-chart" aria-label="Footprint trend chart">
          <div className="panel-header">
            <span className="panel-icon">📈</span>
            <div>
        ...
        <section className="panel panel-3d" aria-label="3D bar chart">
          <div className="panel-header">
            ...
            <div>
              ...
              <div className="panel-sub">Last 10 entries — hover bars for details</div>
            </div>
          </div>
      ...
      <footer className="footer">
        ...
        <p className="footer-copy">
          Powered by FastAPI · React · Three.js · Chart.js &nbsp;|&nbsp; Built with 💚 by Vishnu Vardhan Kosuru
        </p>
      </footer>
    </>
  );
